  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-10 md:px-6 md:py-14">
      <div className="flex items-center justify-between gap-3">
        ...
        <Link href="/admin" className="text-sm underline text-ink-muted hover:text-ink">
          ← Volver al listado
        </Link>
      </div>
      ...
      <div className="mt-6 grid gap-3 rounded border border-neutral-200 bg-white p-4 text-sm sm:grid-cols-2">
        ...
        <p><strong>Total COP:</strong> {order.total_cop.toLocaleString("es-CO")}</p>
        <p><strong>Referencia fuente:</strong> {order.source_reference ?? "-"}</p>
        <p><strong>Creada:</strong> {new Date(order.created_at).toLocaleString("es-CO")}</p>
        <p><strong>Actualizada:</strong> {new Date(order.updated_at).toLocaleString("es-CO")}</p>
      </div>
      ...
      <div className="mt-6 rounded border border-neutral-200 bg-white p-4 text-sm">
        ...
        <ul className="mt-2 space-y-1 text-ink-muted">
          <li><strong className="text-ink">payment_id:</strong> {order.payment.id ?? "-"}</li>
          <li><strong className="text-ink">status:</strong> {order.payment.status ?? "-"}</li>
          <li><strong className="text-ink">normalized_status:</strong> {order.payment.normalized_status ?? "-"}</li>
          <li><strong className="text-ink">status_detail:</strong> {order.payment.status_detail ?? "-"}</li>
          <li><strong className="text-ink">payment_method_id:</strong> {order.payment.payment_method_id ?? "-"}</li>
        </ul>
      </div>
